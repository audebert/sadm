...
USERS = {
    "mdb": {
        "uid": 20000,
        "groups": ("mdb", "mdb_public", "mdbsync", "mdbsync_public", "udbsync_public"),
    },
    "mdbsync": {"uid": 20010, "groups": ("mdbsync", "mdbsync_public", "mdb_public")},
    ...
    "presencesync": {
        "uid": 20060,
        "groups": ("presencesync", "presencesync_public", "mdb_public", "udb_public"),
    },
    "presenced": {
        "uid": 20070,
        "groups": ("presenced", "presencesync", "presencesync_public"),
    },
    "udb": {"uid": 20080, "groups": ("udb", "udb_public", "udbsync", "udbsync_public")},
    "udbsync": {
        "uid": 20090,
    ...
    "cluster": {"uid": 20160, "groups": ("cluster", "cluster_public", "isolate")},
}
...
def install_postgresql():
    ...
    if not os.path.exists(os.path.join(pg_path, "postgresql.conf")):
        system('su - postgres -c "initdb --locale en_US.UTF-8 -D {}"'.format(pg_path))
    install_cfg(
...
def install_nginxcfg():
    install_cfg("nginx/nginx.conf", "/etc/nginx", owner="root:root", mode=0o644)
...
def install_bindcfg():
    ...
    for zone in ("0.in-addr.arpa", "127.in-addr.arpa", "255.in-addr.arpa", "localhost"):
        install_cfg(
            "named/%s.zone" % zone, "/etc/named", owner="named:named", mode=0o640
        )
    install_cfg("named/root.hint", "/etc/named", owner="named:named", mode=0o640)
...
def install_dhcpdcfg():
    install_cfg("dhcpd.conf", "/etc", owner="dhcp:dhcp", mode=0o640)
    mkdir("/etc/dhcpd", mode=0o770, owner="dhcp:mdbdhcp")
    # Create an empty file with the right owner
    copy("/dev/null", "/etc/dhcpd/generated.conf", owner="dhcp:dhcp", mode=0o640)


def install_sshdcfg():
    install_cfg(
        "ssh/sshd_config", "/etc/ssh", owner="root:root", mode=0o644, replace=True
    )
...
def install_mdb():
    ...
    install_cfg("ansible/ansible.cfg", "/etc/ansible/", owner="ansible:ansible")
...
def install_docs():
    ...
    install_service_dir("webservices/docs", mode=0o755, owner="webservices:http")
...
def install_paste():
    ...
    install_service_dir("webservices/paste", mode=0o755, owner="webservices:http")
...
def install_homepage():
    ...
    install_service_dir("django/homepage", owner="homepage:homepage", mode=0o700)
...
def install_concours():
    ...
    install_service_dir("django/concours", owner="concours:concours", mode=0o700)
...
def install_presenced():
    ...
    install_service_dir(
        "python-lib/prologin/presenced", owner="presenced:presenced", mode=0o700
    )
    install_systemd_unit("presenced")

    cfg = "/etc/pam.d/system-login"
    cfg_line = (
        "session requisite pam_exec.so" " /var/prologin/presenced/pam_presenced.py"
    )
...
def install_presencesync_sso():
    ...
    install_cfg_profile("presencesync-sso", group="presencesync_sso")
...
def install_rfs():
    ...
    install_cfg(
        "exports.d/rfs.exports", "/etc/exports.d", mode=0o640, owner="root:root"
    )
    system("exportfs -arv")
...
def install_resource_limits():
    ...
    for fname in (
        ...
        copy(f"etc/systemd/system/{fname}", f"/etc/systemd/system/{fname}", 0o644)
...
def install_sddmcfg():
    ...
    copy("etc/sddm/scripts/Xsetup", "/usr/share/sddm/scripts/Xsetup", mode=0o755)
    copytree(
...
def install_systemd_networkd_rhfs():
    _install_systemd_networkd(["10-rhfs-a.link", "10-rhfs-b.link", "10-rhfs.network"])
...
def install_masternode():
    ...
    mkdir("/var/prologin/concours_shared", mode=0o770, owner="concours:cluster_public")
    mkdir(
...
def install_prometheus():
    for fname in ["prometheus.yml", "alert.rules.yml", "sadm.rules.yml"]:
        install_cfg(
            f"prometheus/{fname}", "/etc/prometheus/", owner="prometheus:prometheus"
        )
