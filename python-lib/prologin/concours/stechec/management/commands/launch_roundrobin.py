...
class Command(BaseCommand):
    ...
    def gen_matches(self, tournament, repeat=1):
        if settings.STECHEC_USE_MAPS and not tournament.maps.all():
            raise RuntimeError(
                "Configured to run with maps but no maps " "present in tournament."
            )

        matches = []
        for chs in itertools.product(
            tournament.players.all(), repeat=settings.STECHEC_NPLAYERS
        ):
    ...
    def launch(self, tournament_id, repeat=1):
        ...
        print(
            "You are about to schedule {} matchs for {} champions "
            "on {} maps.".format(
                len(matches), tournament.players.count(), tournament.maps.count()
            )
        )
