...
class AllChampionsView(ChampionsListMixin, ListView):
    ...
    def get(self, request, *args, **kwargs):
        if settings.STECHEC_FIGHT_ONLY_OWN_CHAMPIONS and not self.request.user.is_staff:
            return HttpResponseForbidden()
        return super().get(request, *args, **kwargs)
...
class MyMatchesView(LoginRequiredMixin, RedirectView):
    ...
    def get_redirect_url(self, *args, **kwargs):
        return "{}?author={}".format(reverse("matches-all"), self.request.user.pk)
...
class ConfirmDeleteChampion(LoginRequiredMixin, DetailView):
    ...
    def get_object(self, queryset=None):
        return get_object_or_404(
            self.model, pk=self.kwargs[self.pk_url_kwarg], author=self.request.user
        )
...
class ChampionSources(LoginRequiredMixin, SingleObjectMixin, View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        h["Content-Disposition"] = "attachment; filename=champion-{}.tgz".format(
    ...
    def get_queryset(self):
        ...
        my_score = (
            models.TournamentPlayer.objects.filter(
                tournament=OuterRef("pk"), champion__author=self.request.user.id
            )
            .order_by()
            .values("score")
        ...
        winner = (
            ...
            .annotate(
                max_score=Max(
                    "tournamentplayers__score", filter=Q(tournamentplayers__score__gt=0)
                )
            )
...
class TournamentViewMixin:
    ...
    def player_queryset(self):
        ...
        if self.jury_first:
            qs = qs.annotate(
                jury_status=Coalesce("correction__include_jury_report", Value(False))
...
class TournamentMatchesView(DetailView):
    ...
    def matches(self):
        ...
        return (
            models.Match.objects.filter(status="done", tournament=tournament.pk)
            .annotate(max_score=Max("matchplayers__score"))
            .filter(players__id=champion.pk)
            .annotate(
                my_score=Max(
                    "matchplayers__score", filter=Q(matchplayers__champion=champion)
                )
            )
            .annotate(
                result=Case(
    ...
    def match_matrix(self):
        ...
        matrix = [{"enemy": e, "matches": against[e.champion_id]} for e in enemies]
        return matrix
...
class RedmineIssueListView(RedirectView):
    ...
    def get_redirect_url(self, *args, **kwargs):
        ...
        return "{}?{}".format(
            settings.STECHEC_REDMINE_ISSUE_LIST, urllib.parse.urlencode(qs, doseq=True)
        )
