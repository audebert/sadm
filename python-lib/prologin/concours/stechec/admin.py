...
class MatchPlayerInline(admin.TabularInline):
    ...
    def formfield_for_foreignkey(self, db_field, request, **kwargs):
        if db_field.name == "champion":
            kwargs["queryset"] = models.Champion.objects.select_related("author")
...
class TournamentMapInline(admin.TabularInline):
    ...
    def get_queryset(self, request):
        return super().get_queryset(request).select_related("map__author", "tournament")
...
class TournamentAdmin(admin.ModelAdmin):
    ...
    def save_related(self, request, form, formsets, change):
        ...
        if form.cleaned_data.get("auto_add"):
            ...
            all_chs = models.Champion.objects.filter(status="ready", deleted=False)
            if deadline:
            ...
            chs_ids = (
                all_chs.values("author__id").annotate(max_id=Max("id")).values("max_id")
            )
