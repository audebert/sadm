...
class LoginMixin:
    ...
    def login(self, user):
        self.assertTrue(self.client.login(username=user.username, password="pswd"))
...
class ChampionTestCase(StechecMixin, LoginMixin, TestCase):
    ...
    def test_upload_champion(self):
        ...
        self.assertContains(self.client.get("/champions/all/"), self.champion_name)
        self.assertContains(
            self.client.get("/champions/mine/", follow=True), self.champion_name
        )
    ...
    def test_delete_champion(self):
        ...
        self.assertNotContains(self.client.get("/champions/mine/"), self.champion_name)
...
class MapTestCase(StechecMixin, LoginMixin, TestCase):
    ...
    def test_add_map(self):
        ...
        r = self.client.post(
            "/maps/new/", {"name": "Such map", "contents": "Much contents, wow"}
        )
