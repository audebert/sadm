...
class Champion(ExportModelOperationsMixin("champion"), models.Model):
    ...
    def source_contents(self):
        ...
        ext_whitelist = set(itertools.chain(*(l["exts"] for l in LANGUAGES.values())))
        file_blacklist = [
            "interface",
            "api",
            "constant",
        ...
        with self._extract_sources() as tmpd:
            for entry in os.scandir(tmpd):
                if not entry.is_file():
                    continue
                if not any(entry.name.lower().endswith(ext) for ext in ext_whitelist):
                    continue
                if any(entry.name.lower().startswith(bf) for bf in file_blacklist):
                    continue
...
class Tournament(ExportModelOperationsMixin("tournament"), models.Model):
    ...
    maps = models.ManyToManyField(
        Map, verbose_name="maps", related_name="tournaments", through="TournamentMap"
    )
...
class TournamentPlayer(ExportModelOperationsMixin("tournament_player"), models.Model):
...
class TournamentMap(ExportModelOperationsMixin("tournament_map"), models.Model):
    map = models.ForeignKey(Map, on_delete=models.CASCADE, verbose_name="carte")
    tournament = models.ForeignKey(
...
class MatchPlayer(ExportModelOperationsMixin("match_player"), models.Model):
    ...
    has_timeout = models.BooleanField(default=False, verbose_name="has timeout")
