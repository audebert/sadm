...
class UserViewSet(viewsets.ReadOnlyModelViewSet):
    queryset = (
        get_user_model().objects.all().prefetch_related("maps", "champions", "matches")
    )
...
class TournamentViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def get_stats(self):
        ...
        champions = {}
        tournaments = tournament.tournamentplayers.prefetch_related("champion__author")
    ...
    def evolution(self, request):
        ...
        users = {c.author.username for t in tournaments for c in t.players.all()}
        rankings = {u: [None] * len(tournaments) for u in users}
        for tid, tournament in enumerate(tournaments):
            players = sorted(tournament.tournamentplayers.all(), key=lambda p: -p.score)
