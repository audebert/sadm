...
if "shared_secret" not in CLT_CFG:
    raise RuntimeError("Missing shared_secret in the presenced-client YAML config")
...
class SendHeartbeatHandler(tornado.web.RequestHandler):
    ...
    def post(self, msg):
        ...
        if len(logins) == 0:
            ...
            self.application.presencesync.send_heartbeat(logins.pop(), HOSTNAME)
            self.set_status(200, "OK, one user logged in")
        else:
            logging.error("There are too many users logged in: %s", ", ".join(logins))
...
class PresencedServer(prologin.web.TornadoApp):
    def __init__(self, secret, port):
        super(PresencedServer, self).__init__(
            [(r"/send_heartbeat", SendHeartbeatHandler), (r"/login", LoginHandler),],
            "presenced",
        )
