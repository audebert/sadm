...
class WorkerNode(prologin.rpc.server.BaseRPCApp):
    ...
    async def send_heartbeat(self):
        logging.debug(
            "sending heartbeat to the server, %s/%s slots", self.slots, self.max_slots
        )
        first_heartbeat = True
        while True:
            try:
                await self.master.heartbeat(self.get_worker_infos(), first_heartbeat)
                first_heartbeat = False
            except socket.error:
                logging.warning("master down, retrying heartbeat in %ss", self.interval)
    ...
    async def compile_champion(self, user, cid, ctgz):
        ...
        ret, compiled, log = await operations.compile_champion(self.config, ctgz)
        try:
    ...
    async def run_match(self, match_id, players, map_contents=None):
        ...
        workernode_run_match_summary.observe(max(time.monotonic() - run_match_start, 0))
