...
def get_hfs_ip(hfs_id):
    ...
    potential_hfs = prologin.mdb.client.connect().query(aliases__contains=hfs_alias)
...
class HFSRequestHandler(http.server.BaseHTTPRequestHandler):
    ...
    def start_nbd_server(self, filename):
        ...
        cmd = ["nbd-server", "-p", "/tmp/nbd.%s.pid" % self.user, "-C", config_file]
    ...
    def new_user_handler(self):
        ...
        hfs_new_user.labels(user=self.user, user_type=self.user_type).observe(delta)
    ...
    def remote_user_handler(self, peer_id):
        ...
        tar = subprocess.Popen(tar_cmdline, stdin=subprocess.PIPE, cwd=nbd_path.parent)
        try:
            BLOCK_SIZE = 65536
        ...
        hfs_migrate_remote_user.labels(user=self.user, hfs=peer_id).observe(delta)
