...
if "shared_secret" not in PUB_CFG:
    raise RuntimeError("Missing shared_secret in the presencesync-pub YAML config")

if "shared_secret" not in SUB_CFG:
    raise RuntimeError("Missing shared_secret in the presencesync-sub YAML config")
...
class TimeoutedPubSubQueue(prologin.synchronisation.BasePubSubQueue):
    ...
    def is_login_allowed(self, login, hostname):
        ...
        if self.backlog.get(login, (None, None))[1] == hostname:
            presencesync_login_failed.labels(user=login, reason="already_logged").inc()
    ...
    def request_login(self, login, hostname):
        ...
        if self.start_ts is None or time.time() < self.start_ts + self.TIMEOUT:
            logging.debug("Login for %s on %s refused: too early", login, hostname)
            ...
            presencesync_login_failed.labels(user=login, reason="too_early").inc()
...
class HeartbeatHandler(tornado.web.RequestHandler):
    ...
    def post(self, msg):
        msg = json.loads(msg)
        self.application.pubsub_queue.update_backlog(msg["login"], msg["hostname"])
        self.set_status(200, "OK")
...
if __name__ == "__main__":
    ...
    server = SyncServer(PUB_CFG["shared_secret"], SUB_CFG["shared_secret"], port)
    server.start()
